...
/////////////////////////////////////////////////
// User-Defined Functions for Estimation Module
//

function getIterationCount(phase) {
	try {
		var elements = 0;
		for (var i = 0; i < phase.GetAssociationCount(); i++) {
			var elem = phase.GetAssociationAt(i).GetOtherSide().Participant;
			if (elem.IsKindOf("UMLClass") && elem.StereotypeName == "Iteration") {
				elements++;
			}
		}
	} catch(e) {
...
function getArtifactsNames(iteration) {
	try {
		var names = "";
		for (var i = 0; i < iteration.GetClientDependencyCount(); i++) {
			var elem = iteration.GetClientDependencyAt(i);
			if (elem.StereotypeName == "Generate" && elem.Supplier.IsKindOf("UMLArtifact")) 
			{
				if (trim(names) != "") {
					names = names + "\r\n" + elem.Supplier.Name;
				} 
				else {
					names = elem.Supplier.Name;
				}
			}
		}
	} catch(e) {
...
function getUseCaseNames(iteration) {
	try {
		var names = "";
		for (var i = 0; i < iteration.GetTaggedValueAsReferenceCount("Estimation","Iteration Estimation Values","Use Case List"); i++) {
			var elem = iteration.GetTaggedValueAsReferenceAt("Estimation","Iteration Estimation Values","Use Case List",i);
			if (elem.IsKindOf("UMLUseCase")) {
				if (trim(names) != "") {
					names = names + "\r\n" + elem.Name;
				}
				else {
...
function getNoneOrContent(NoneString, ContentString) {
...
function getActorNames(UseCase) {
	try {
		var names = "";
		for (var i = 0; i < UseCase.GetAssociationCount(); i++) {
			var elem = UseCase.GetAssociationAt(i).GetOtherSide().Participant;
			if (elem.IsKindOf("UMLActor")) {
				if (trim(names) != "") {
